...
export function Income({ income, familyMembers: rawFamilyMembers, onSave }: IncomeProps) {
  ...
  // Filter income by selected owner
  ...
  async function handleToggleActive(source: IncomeSource) {
    const updated = income.map((s) => (s.id === source.id ? { ...s, active: !s.active } : s))
    await onSave(updated)
  }

  const isFormValid = form.name.trim() && parseFloat(form.grossAmount) > 0 && parseFloat(form.netAmount) > 0
    ...
    <div className="px-8 py-8">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        ...
        <SummaryCard
          ...
          sub={income.length - activeIncome.length > 0 ? `${income.length - activeIncome.length} paused` : 'all active'}
          plain
        />
      </div>
      ...
      {income.length === 0 ? (
        ...
        <div className="space-y-4">
          {Object.entries(byOwner)
            ...
            .map(([owner, sources]) => {
              return (
                <div key={owner} className="bg-[#14141f] border border-white/5 rounded-xl overflow-hidden">
                  {/* Owner header */}
                  <div className="px-5 py-3.5 border-b border-white/5 bg-white/[0.02]">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs text-gray-600 bg-white/5 rounded-full px-2 py-0.5">{sources.length}</span>
                    </div>
                  </div>
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className={cn('text-sm font-medium', source.active ? 'text-gray-200' : 'text-gray-500 line-through')}>
                              {source.name}
                            </p>
                            ...
                            <div className="flex items-baseline gap-2">
                              <div className="flex flex-col items-start">
                                <div className="flex items-baseline gap-1">
                                  <span className="text-sm font-semibold text-green-400">{formatCurrency(source.grossAmount, currency)}</span>
                              ...
                              <div className="flex items-baseline gap-1">
                                <span className="text-sm font-semibold text-green-300">{formatCurrency(source.netAmount, currency)}</span>
